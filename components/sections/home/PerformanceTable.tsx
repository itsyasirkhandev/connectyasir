                ...
                {data.map((row, i) => (
                  <tr key={i} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-1 text-[10px] font-bold tracking-wider uppercase ${row.status === 'Optimal' ? 'bg-green-500/10 text-green-800 dark:text-green-400' : 'bg-muted text-foreground/70 dark:text-muted-foreground'}`}
                      >
                        {row.status}
                      </span>
                    </td>
